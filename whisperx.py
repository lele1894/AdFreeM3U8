...
def translate_batch(texts, target_lang='zh-CN', batch_size=10):
    """批量翻译文本"""
    results = []
    translator = GoogleTranslator(source='auto', target=target_lang)
    ...
    try:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            translated = [translator.translate(text) for text in batch]
            results.extend(translated)
            
    except Exception as e:
        print(f"批量翻译出错，切换到单条翻译: {str(e)}")
        # 如果批量翻译失败，切换到单条翻译
        for text in texts[len(results):]:
            try:
                ...
                print(f"单条翻译出错: {str(e)}")
                results.append(text)  # 如果翻译失败，使用原文
...
async def generate_speech(text, output_file, voice="zh-TW-HsiaoChenNeural", rate="+50%"):
    """使用Edge TTS生成语音"""
...
def mix_audio_and_merge_video(video_path, dubbed_audio_path, output_dir):
    ...
    try:
        ...
        # 确保两个音频长度相同
        max_length = max(len(original_audio), len(dubbed_audio))
        if len(original_audio) < max_length:
            original_audio += AudioSegment.silent(duration=max_length - len(original_audio))
        if len(dubbed_audio) < max_length:
            dubbed_audio += AudioSegment.silent(duration=max_length - len(dubbed_audio))
        
        # 混合音频
        mixed_audio = original_audio.overlay(dubbed_audio)
        
        # 4. 保存混合后的音频
        mixed_audio_path = os.path.join(output_dir, "mixed_audio.wav")
        mixed_audio.export(mixed_audio_path, format="wav", parameters=["-ar", "44100", "-ac", "2"])
...
def process_video(video_path, model_name="large-v2", device="cuda", batch_size=16, compute_type="float16", target_lang='zh-CN', voice="zh-TW-HsiaoChenNeural", rate="+50%"):
    """处理视频并生成字幕和配音"""
